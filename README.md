...
## Key Features:
- User can browse the movies through carousel.
- User can watch the trailers of the movies.
- User can create review for the movies.
- User can see the prevously written reviews.
...
### Code snippets:
Code below is reponsible for updates the reviewIds array in the Movie collection. Each movie initially contains an empty reviewIds array. When a new review is created, the MongoTemplate performs an update operation, matching the movie in the database by its imdbId. The reviewIds array is updated by pushing the newly created review's ID, ensuring that the review is correctly linked to the corresponding movie.
```
